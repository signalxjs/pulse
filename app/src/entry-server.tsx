 ...
 * user — in one of two shapes: the resolved context itself (prod handler,
 * tests), or a request carrying it as `pulseCtx` (dev — the handler
 * forwards the raw IncomingMessage since @sigx/vite 0.13.0, core#304). The
 * user rides DI (useRequestUser) so the auth guard sees it pre-render, and
 * App's setup seeds the session store during component resolution (the
 * pattern store 0.11 documents for request state — store#63). Signed-out
 * semantics are `user: null`. Page DATA no longer travels here: components
 * call server functions directly, which resolve this request from the
 * ambient scope the document handler opens (rfc-server §7).
 */
